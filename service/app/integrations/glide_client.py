...
class GlideClient:
    ...
    def _post_querytables(self, payload: Dict[str, Any], *, timeout: int) -> Any:
        s = self.settings
    ...
    def list_table_rows(self, table_name: str, *, timeout: int = 60) -> list[dict]:
        """
        Fetch all rows from any Glide table using tableName query (no SQL).
        Handles pagination via 'next'.
        """
        if not self.enabled():
            return []
        ...
        while True:
            q: Dict[str, Any] = {"tableName": table_name, "utc": True}
            if start_at:
                q["startAt"] = start_at

            payload = {"appID": s.glide_app_id, "queries": [q]}
            data = self._post_querytables(payload, timeout=timeout)
    ...
    def get_row_by_row_id(
        ...
        *,
        ...
        """
        SQL query to fetch a single row by Row ID from any table.
        """
        if not self.enabled():
            return None
        ...
        s = self.settings
        sql = f'SELECT * FROM "{table_name}" WHERE "{rowid_column}" = $1 LIMIT 1'
        payload = {"appID": s.glide_app_id, "queries": [{"sql": sql, "params": [row_id]}]}
        data = self._post_querytables(payload, timeout=timeout)
